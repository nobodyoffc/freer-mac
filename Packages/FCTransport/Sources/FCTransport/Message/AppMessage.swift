...
public struct AppMessageEnvelope: Equatable, Sendable {
    ...
    public init(type: MessageType, messageId: Int64, flags: Flags = [], payload: Data) {
        ...
        self.flags = flags
        // Direct assignment, not `Data(payload)`: Data's CoW already
        // gives value semantics, and a decoded payload may be a slice
        // of a file-mapped download that must not be copied into
        // anonymous memory. NOTE: as a slice it may have a non-zero
        // startIndex — consumers must index relative to it.
...
public enum AppMessageCodec {
    ...
    /// Decode an envelope. The returned payload is a **slice** sharing
    /// the input's backing storage — for a file-mapped multi-hundred-MB
    /// message (large download reassembled via disk spill) no anonymous
    /// copy is made. Callers that need the payload to outlive `data`
    /// copy it themselves.
    public static func decode(_ data: Data) throws -> AppMessageEnvelope {
        guard data.count >= minimumLength else {
            throw Failure.truncated(needed: minimumLength, got: data.count)
        }
        let base = data.startIndex

        let typeByte = data[base]
        guard let type = MessageType(rawValue: typeByte) else {
            throw Failure.unknownType(typeByte)
        }

        var id: UInt64 = 0
        for i in 0..<8 { id = (id << 8) | UInt64(data[base + 1 + i]) }
        let messageId = Int64(bitPattern: id)
        let flags = AppMessageEnvelope.Flags(rawValue: data[base + 9])

        // Decode the varint starting at offset 10.
        let (rawLen, consumed): (UInt64, Int)
        do {
            (rawLen, consumed) = try FudpVarint.decode(data[(base + 10)...])
        } catch let e as FudpVarint.Failure {
            throw Failure.varint(e)
        }
        let payloadLen = Int(rawLen)

        let payloadStart = base + 10 + consumed
        let available = data.endIndex - payloadStart
