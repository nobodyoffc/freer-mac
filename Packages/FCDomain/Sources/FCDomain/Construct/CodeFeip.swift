...
public enum CodeFeip {
    ...
    public enum Failure: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .rateOutOfRange(let r):
                return "CodeFeip: a rating is 0 to 5, not \(r)"
    ...
    /// `{"op":"rate","codeId":…,"rate":n,"cause":"…"}` — score a code record 0–5.
    ///
    ...
    /// ``ActiveSession/carveCodeRateOnChain(codeId:rate:cause:weightCd:feePerByte:timeoutMs:)``, which takes the weight as a
    ...
    /// could express a 1.
    public static func rateOp(codeId: String, rate: Int, cause: String? = nil) throws -> String {
        guard !codeId.isEmpty else { throw Failure.noCodeId }
        guard (0...5).contains(rate) else { throw Failure.rateOutOfRange(rate) }
        var data: [String: Any] = ["op": Op.rate.rawValue, "codeId": codeId, "rate": rate]
    ...
    /// selected.
    public static func rateCarve(codeId: String, rate: Int, cause: String? = nil) throws -> String {
        try sized(envelope(opJson: rateOp(codeId: codeId, rate: rate, cause: cause)))
    }
    ...
    /// spends more than one byte. ``rateCarve(codeId:rate:cause:)`` is
    /// the authority; this is what to draw a counter against.
    public static func remainingCauseBytes(codeId: String, rate: Int, cause: String) -> Int {
        guard let full = try? envelope(opJson: rateOp(codeId: codeId, rate: rate, cause: cause)).utf8.count else { return 0 }
