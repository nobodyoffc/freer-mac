import Foundation
import FCCore
import FCTransport

/// On-chain protocol reads — the port of Android's `ProtocolManager`'s
/// API half (`makeFcdsl` / `makeFcdslForOwner` / `buildSearchFcdsl` /
/// `fetchPageProtocols`), over `base.search` against the `protocol`
/// index.
///
/// The write half — publish, update, stop, recover, close — lives on
/// ``ActiveSession`` next to the other carves, because it goes through
/// the wallet's send pipeline rather than this client.
///
/// **This index is chain-wide, unlike ``ProofService``'s.** A proof
/// query is scoped to your FID because a pane titled "your proofs" that
/// returned strangers' documents would be a different feature wearing
/// this one's UI. A protocol registry is the opposite: the whole point
/// is to look up protocols *other people* published. So the browse is
/// unscoped by default and `owner` is a filter you opt into — Android's
/// More ▸ My protocols.
///
/// **No key is needed to read.** Every field of a published protocol is
/// public, which makes this, like ``NewsService``, a path that works
/// fully on a watch-only identity. What you cannot do from there is
/// publish one.
public struct ProtocolService {

    /// The index this reads. Java's `IndicesNames.PROTOCOL`.
    public static let index = "protocol"

    public enum Failure: Error, CustomStringConvertible {
        case fapiNonZeroCode(api: String, code: Int, message: String?)
        case underlying(Error)

        public var description: String {
            switch self {
            case let .fapiNonZeroCode(api, code, message):
                return "ProtocolService: \(api) returned code=\(code) message=\(message ?? "<nil>")"
            case .underlying(let e):
                return "ProtocolService: \(e)"
            }
        }
    }

    /// A field the `protocol` index can be searched or sorted on. The
    /// two sets differ — you can search the description but not sort by
    /// it — so ``searchable`` and ``sortable`` name them separately,
    /// mirroring Java's `getSearchableFields` / `getSortableFields`.
    public enum Field: String, CaseIterable, Sendable, Identifiable {
        case name
        case type
        case sn
        case ver
        case did
        case desc
        case lang
        case owner
        case waiters
        case birthTime
        case lastTime
        case lastHeight
        case tCdd
        case tRate
        case id

        public var id: String { rawValue }

        /// The wire name — what goes in the fcdsl.
        public var wire: String { rawValue }

        /// Display label. English only for now; Phase 11 localises.
        public var label: String {
            switch self {
            case .name:       return "Name"
            case .type:       return "Type"
            case .sn:         return "Serial number"
            case .ver:        return "Version"
            case .did:        return "DID"
            case .desc:       return "Description"
            case .lang:       return "Language"
            case .owner:      return "Owner"
            case .waiters:    return "Waiter"
            case .birthTime:  return "Published"
            case .lastTime:   return "Time"
            case .lastHeight: return "Height"
            case .tCdd:       return "CDD"
            case .tRate:      return "Rating"
            case .id:         return "ID"
            }
        }

        /// What Java's `getSearchableFields` lists, minus `home` — a
        /// map of label→URL that a `match` clause has no sensible way to
        /// score — plus `id`, so pasting one finds its row.
        public static let searchable: [Field] =
            [.name, .type, .sn, .desc, .lang, .owner, .waiters, .id]

        /// Java's `getSortableFields`, minus the two booleans (`active`
        /// and `closed` sort a list into two undifferentiated blocks,
        /// which is a filter wearing a sort's clothes — the state
        /// filters do that job properly).
        public static let sortable: [Field] =
            [.lastHeight, .lastTime, .birthTime, .name, .owner, .tCdd, .tRate, .id]
    }

    /// One page of results plus what is needed to ask for the next.
    public struct Page: Sendable {
        public let protocols: [ProtocolSpec]
        /// The server's own cursor for the page after this one. Feed it
        /// straight back as `after` — see ``search`` for why this rather
        /// than a locally rebuilt key pair.
        public let last: [String]?
        public let total: Int64?
        public let bestHeight: Int64?

        public init(
            protocols: [ProtocolSpec], last: [String]?,
            total: Int64?, bestHeight: Int64?
        ) {
            self.protocols = protocols
            self.last = last
            self.total = total
            self.bestHeight = bestHeight
        }
    }

    public let fapi: any FapiCalling

    public init(fapi: any FapiCalling) {
        self.fapi = fapi
    }

    // MARK: - browse

    /// One page of the registry, newest first.
    ///
    /// `owner` nil browses every protocol on the chain; passing a FID
    /// narrows it to that publisher's — Android's `makeFcdslForOwner`.
    /// `active` and `closed` are three-valued: nil leaves the state out
    /// of the query entirely, which is how the All tab sees stopped and
    /// closed records alongside live ones.
    public func fetchProtocols(
        owner: String? = nil,
        active: Bool? = nil,
        closed: Bool? = nil,
        ascending: Bool = false,
        after: [String]? = nil,
        size: Int = 25,
        timeoutMs: Int = 15_000
    ) async throws -> Page {
        let order = ascending ? "asc" : "desc"
        var query: [String: Any] = [:]
        if let owner, !owner.isEmpty {
            query["equals"] = ["fields": [Field.owner.wire], "values": [owner]]
        }
        let state = Self.stateClauses(active: active, closed: closed)
        if let terms = state.queryTerms { query["terms"] = terms }

        var dict: [String: Any] = [
            "entity": Self.index,
            "sort": [
                ["field": Field.lastHeight.wire, "order": order],
                ["field": Field.id.wire,         "order": order]
            ],
            "size": String(size)
        ]
        if !query.isEmpty { dict["query"] = query }
        if let filter = state.filter { dict["filter"] = filter }
        if let after, !after.isEmpty { dict["after"] = after }
        return try await run(dict, timeoutMs: timeoutMs)
    }

    // MARK: - search

    /// Full-text search over the registry.
    ///
    /// **Pages from the server's cursor.** Android rebuilds `after` as
    /// `[lastHeight, id]` for every search regardless of what it sorted
    /// by — and worse, `addSortingToFcdsl` defaults to sorting on
    /// `lastTime` while `makeSortList` builds the cursor from
    /// `lastHeight`, so even its *default* search pages against keys it
    /// is not sorted on. `search_after` compares positionally against
    /// the sort keys actually used, so those searches page wrong (the
    /// same defect as News and Proof, **Android issue C17**). Passing
    /// ``Page/last`` back is correct for every sort.
    public func search(
        query text: String,
        inField: Field? = nil,
        sortField: Field? = nil,
        ascending: Bool = false,
        owner: String? = nil,
        active: Bool? = nil,
        closed: Bool? = nil,
        after: [String]? = nil,
        size: Int = 25,
        timeoutMs: Int = 15_000
    ) async throws -> Page {
        let fields = inField.map { [$0.wire] } ?? Field.searchable.map(\.wire)
        let order = ascending ? "asc" : "desc"
        let sorted = sortField ?? .lastHeight

        var sorts: [[String: String]] = [["field": sorted.wire, "order": order]]
        if sorted != .id {
            sorts.append(["field": Field.id.wire, "order": order])
        }

        var query: [String: Any] = [
            "match": ["fields": fields, "value": text]
        ]
        if let owner, !owner.isEmpty {
            query["equals"] = ["fields": [Field.owner.wire], "values": [owner]]
        }
        let state = Self.stateClauses(active: active, closed: closed)
        if let terms = state.queryTerms { query["terms"] = terms }

        var dict: [String: Any] = [
            "entity": Self.index,
            "query": query,
            "sort": sorts,
            "size": String(size)
        ]
        if let filter = state.filter { dict["filter"] = filter }
        if let after, !after.isEmpty { dict["after"] = after }
        return try await run(dict, timeoutMs: timeoutMs)
    }

    /// The two lifecycle flags, as fcdsl clauses.
    ///
    /// **They cannot share one `terms`.** A single clause naming two
    /// fields and two values matches either value in either field, so
    /// asking for `active=true, closed=false` that way would also return
    /// `active=false, closed=true` — precisely the rows being excluded.
    /// So `active` goes in the query and `closed` in the top-level
    /// `filter`, which is a sibling of `query` in the fcdsl (Java's
    /// `Fcdsl.filter`), not a member of it. A filter nested inside the
    /// query is silently ignored, and a state filter that does nothing
    /// looks exactly like one that worked on a chain where every record
    /// happens to be live.
    ///
    /// The values go as strings — the index stores the boolean, the
    /// query spells it, the same way ``DirectoryService`` asks for
    /// active services.
    private static func stateClauses(
        active: Bool?, closed: Bool?
    ) -> (queryTerms: [String: Any]?, filter: [String: Any]?) {
        let queryTerms: [String: Any]? = active.map {
            ["fields": ["active"], "values": [$0 ? "true" : "false"]]
        }
        let filter: [String: Any]? = closed.map {
            ["terms": ["fields": ["closed"], "values": [$0 ? "true" : "false"]]]
        }
        return (queryTerms, filter)
    }

    // MARK: - by id

    /// Fetch specific protocols by record id — Android's
    /// `protocolByIds`.
    ///
    /// This is the path for the two things the browse query cannot do:
    /// read back a protocol you have only the id of (a `prePid` link, or
    /// one somebody sent you), and confirm a carve you just broadcast —
    /// the record appears here once a block includes it, which is what
    /// flips a row from broadcast-unconfirmed to on-chain.
    ///
    /// **Not `base.search`.** By-ids is its own endpoint
    /// (``DirectoryService/getByIdsApi``) and its `data` is a *map* from
    /// id to record, not an array. Ids absent from the reply simply have
    /// no record yet, which for a just-broadcast carve is the expected
    /// answer.
    public func fetchProtocolsByIds(
        _ ids: [String],
        timeoutMs: Int = 15_000
    ) async throws -> [String: ProtocolSpec] {
        guard !ids.isEmpty else { return [:] }
        var found: [String: ProtocolSpec] = [:]
        // The endpoint caps how many ids one call may name; page the
        // list rather than sending an unbounded one.
        for start in stride(from: 0, to: ids.count, by: 100) {
            let chunk = Array(ids[start..<min(start + 100, ids.count)])
            let body: Data
            do {
                body = try JSONSerialization.data(
                    withJSONObject: ["entity": Self.index, "ids": chunk],
                    options: [.sortedKeys]
                )
            } catch {
                throw Failure.underlying(error)
            }
            let reply = try await fapi.call(
                api: DirectoryService.getByIdsApi,
                params: nil, fcdsl: body, binary: nil,
                sid: nil, via: nil, maxCost: nil,
                timeoutMs: timeoutMs
            )
            let resp = reply.response
            if let code = resp.code, code != 0 {
                if code == 404 { continue }
                throw Failure.fapiNonZeroCode(
                    api: DirectoryService.getByIdsApi, code: code, message: resp.message
                )
            }
            guard let data = resp.data else { continue }
            let page: [String: ProtocolSpec]
            do {
                page = try JSONDecoder().decode([String: ProtocolSpec].self, from: data)
            } catch {
                throw Failure.underlying(error)
            }
            for (key, row) in page {
                var copy = row
                // The map key is authoritative: a record whose body
                // omits its own id still has one here.
                if copy.id.isEmpty { copy.id = key }
                copy.onChain = true
                found[key] = copy
            }
        }
        return found
    }

    // MARK: - the one call

    private func run(_ fcdsl: [String: Any], timeoutMs: Int) async throws -> Page {
        let body: Data
        do {
            body = try JSONSerialization.data(withJSONObject: fcdsl, options: [.sortedKeys])
        } catch {
            throw Failure.underlying(error)
        }
        let reply = try await fapi.call(
            api: "base.search",
            params: nil, fcdsl: body, binary: nil,
            sid: nil, via: nil, maxCost: nil,
            timeoutMs: timeoutMs
        )
        let resp = reply.response
        // 404 is the server's "nothing matched" — an empty page, and for
        // a filter nothing satisfies, the normal reply.
        if let code = resp.code, code != 0 {
            if code == 404 {
                return Page(protocols: [], last: nil, total: 0, bestHeight: resp.bestHeight)
            }
            throw Failure.fapiNonZeroCode(
                api: "base.search", code: code, message: resp.message
            )
        }
        guard let data = resp.data else {
            return Page(
                protocols: [], last: resp.last,
                total: resp.total, bestHeight: resp.bestHeight
            )
        }
        do {
            let decoded = try JSONDecoder().decode([ProtocolSpec].self, from: data)
            // Chain rows are on-chain by definition; the wire may or may
            // not carry the flag, and the pane's whole layout keys off
            // it, so stamp it rather than trusting the field to be there.
            let protocols = decoded
                .filter { !$0.id.isEmpty }
                .map { row -> ProtocolSpec in
                    var copy = row
                    copy.onChain = true
                    return copy
                }
            return Page(
                protocols: protocols,
                last: resp.last,
                total: resp.total,
                bestHeight: resp.bestHeight
            )
        } catch {
            throw Failure.underlying(error)
        }
    }
}
