...
/// On-chain code-registry reads — the port of Android's `CodeManager`'s
/// API half (`makeFcdsl` / `makeFcdslForOwner` / `buildSearchFcdsl` /
/// `fetchPageCodes`), over `base.search` against the `code` index.
///
...
///
/// **This index is chain-wide, like ``ProtocolService``'s and unlike
/// ``ProofService``'s.** A proof query is scoped to your FID; a registry
/// is worth having for the opposite reason — the point is to look up
/// implementations *other people* published. So the browse is unscoped
/// by default and `owner` is a filter you opt into (Android's More ▸ My
/// codes).
///
/// **No key is needed to read.** Every field of a published code record
/// is public, which makes this a path that works fully on a watch-only
/// identity. What you cannot do from there is publish one.
public struct CodeService {

    /// The index this reads. Java's `IndicesNames.CODE`.
    public static let index = "code"
    ...
    public enum Failure: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case let .fapiNonZeroCode(api, code, message):
                return "CodeService: \(api) returned code=\(code) message=\(message ?? "<nil>")"
            case .underlying(let e):
                return "CodeService: \(e)"
            }
        }
    }

    /// A field the `code` index can be searched or sorted on. The two
    ...
    public enum Field: String, CaseIterable, Sendable, Identifiable {
        ...
        /// What Java's `getSearchableFields` lists, minus `home` — a map
        /// of label→URL that a `match` clause has no sensible way to
    ...
    ///
    /// `owner` nil browses every code record on the chain; passing a FID
    ...
    /// same defect as News, Proof and Protocol, **Android issue C17**).
    /// Passing ``Page/last`` back is correct for every sort.
    public func search(
        query text: String,
        inField: Field? = nil,
        sortField: Field? = nil,
    ...
    /// Fetch specific code records by id.
    ///
    /// This is the path for the two things the browse query cannot do:
    /// read back a record you have only the id of (one somebody sent
    /// you), and confirm a carve you just broadcast — the record appears
    /// here once a block includes it, which is what flips a row from
    /// broadcast-unconfirmed to on-chain.
    ///
    ...
    /// answer.
    public func fetchCodesByIds(
        _ ids: [String],
        timeoutMs: Int = 15_000
    ) async throws -> [String: Code] {
        guard !ids.isEmpty else { return [:] }
        var found: [String: Code] = [:]
        ...
        for start in stride(from: 0, to: ids.count, by: 100) {
            ...
            let page: [String: Code]
            do {
                page = try JSONDecoder().decode([String: Code].self, from: data)
            } catch {
                throw Failure.underlying(error)
            }
            for (key, row) in page {
    ...
    private func run(_ fcdsl: [String: Any], timeoutMs: Int) async throws -> Page {
        ...
        if let code = resp.code, code != 0 {
            if code == 404 {
                return Page(codes: [], last: nil, total: 0, bestHeight: resp.bestHeight)
            }
            throw Failure.fapiNonZeroCode(
        ...
        guard let data = resp.data else {
            return Page(
                codes: [], last: resp.last,
                total: resp.total, bestHeight: resp.bestHeight
            )
        }
        do {
            let decoded = try JSONDecoder().decode([Code].self, from: data)
