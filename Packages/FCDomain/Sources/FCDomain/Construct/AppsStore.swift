...
/// Local app drafts, the cached registry window, and the hidden list —
/// Android's `APP_LIST` entity DB plus its local-deleted list, in one
/// store. The same shape as ``ProtocolsStore``, ``CodesStore`` and
/// ``ServicesStore``, over the `app` index — the fourth and last of the
/// Construct stores.
///
...
/// own activity. The app registry is chain-wide — the window is a slice
...
public struct AppsStore {

    /// Drafts, keyed by ``AppRecord/id``.
    public static let draftNamespace = "apps.drafts.v1"
    /// The single cache row: the registry window and the hidden ids.
    public static let cacheNamespace = "apps.cache.v1"
    ...
    public static let maxCachedApps = 200
    ...
    private let draftStore: TypedStore<AppRecord>
    ...
    public func upsertDraft(_ app: AppRecord) throws {
        var c = app
        c.updatedAt = Date()
        try draftStore.put(c, key: c.id)
    }

    public func draft(id: String) throws -> AppRecord? {
    ...
    /// and no chain time, so ``AppRecord/updatedAt`` is the only ordering
    /// that means anything here.
    public func drafts() throws -> [AppRecord] {
    ...
    /// stamped broadcast-unconfirmed (``AppRecord/onChain`` nil) rather than
    ...
    /// the meantime.
    @discardableResult
    public func promoteDraft(id draftId: String, toTxid txid: String) throws -> AppRecord? {
    ...
    public func saveWindow(_ apps: [AppRecord]) throws {
        try mutate { c in
            ...
            let firstSeen = Dictionary(
                c.apps.map { ($0.id, $0.addedAt) }, uniquingKeysWith: { a, _ in a }
            )
            c.apps = apps.prefix(Self.maxCachedApps).map { row in
                var copy = row
    ...
    /// The cached window.
    public func window() throws -> [AppRecord] {
        try load().apps
    }

    /// The cached window minus the hidden rows.
    public func visibleWindow() throws -> [AppRecord] {
        let c = try load()
        let hidden = Set(c.hiddenIds)
        return c.apps.filter { !hidden.contains($0.id) }
    }
    ...
    /// round.
    public func rememberBroadcast(_ app: AppRecord) throws {
        try mutate { c in
            c.apps.removeAll { $0.id == app.id }
            var row = app
            row.updatedAt = Date()
            c.apps.insert(row, at: 0)
            c.apps = Array(c.apps.prefix(Self.maxCachedApps))
        }
    }
    ...
    public func hidden() throws -> [AppRecord] {
        let c = try load()
        let hidden = Set(c.hiddenIds)
        return c.apps.filter { hidden.contains($0.id) }
    }
