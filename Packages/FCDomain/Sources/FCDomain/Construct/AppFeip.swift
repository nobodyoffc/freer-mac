...
public enum AppFeip {
    ...
    public enum Failure: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .rateOutOfRange(let r):
                return "AppFeip: a rating is 0 to 5, not \(r)"
    ...
    /// `{"op":"rate","aid":…,"rate":n,"cause":"…"}` — score an app 0–5.
    ///
    ...
    /// ``ActiveSession/carveAppRateOnChain(aid:rate:cause:weightCd:feePerByte:timeoutMs:)``, which takes the weight as a
    ...
    /// could express a 1.
    public static func rateOp(aid: String, rate: Int, cause: String? = nil) throws -> String {
        guard !aid.isEmpty else { throw Failure.noAid }
        guard (0...5).contains(rate) else { throw Failure.rateOutOfRange(rate) }
        var data: [String: Any] = ["op": Op.rate.rawValue, "aid": aid, "rate": rate]
    ...
    /// selected.
    public static func rateCarve(aid: String, rate: Int, cause: String? = nil) throws -> String {
        try sized(envelope(opJson: rateOp(aid: aid, rate: rate, cause: cause)))
    }
    ...
    /// spends more than one byte. ``rateCarve(aid:rate:cause:)`` is
    /// the authority; this is what to draw a counter against.
    public static func remainingCauseBytes(aid: String, rate: Int, cause: String) -> Int {
        guard let full = try? envelope(opJson: rateOp(aid: aid, rate: rate, cause: cause)).utf8.count else { return 0 }
