...
status: Phases 1–7 complete; **Phase 8.4 complete** — FUDP streaming (8.4.1), `FileCipher` (8.4.2), `Hat`/`HatsStore` (8.4.3), reference-mode `FileVault` (8.4.4), the `DiskService`/`HatSyncService` two-HAT flows (8.4.5) and the Files pane (8.4.6). **Phase 9 is in progress, Mail (9.1) first and IM (9.2) second** — see the reordering note under Phase 9. 8.3 (application/token/proof/protocol lifecycle) still pending.
...
- End-to-end encryption via `FCCore` (AES-GCM + ECDH) — reuses 9.1.1's AsyTwoWay envelope
