...
status: Phases 1–7 complete; Phase 8 at 8.4.4 — FUDP streaming (8.4.1), `FileCipher` (8.4.2), `Hat`/`HatsStore` (8.4.3) and the reference-mode `FileVault` (8.4.4) shipped; next is 8.4.5 (`DiskService` + `HatSyncService`, the two-HAT upload/download flows); 8.3 (application/token/proof/protocol lifecycle) still pending.
