...
status: Phases 1–7 complete; Phase 8 at 8.4.1 — FUDP large-payload streaming (outbound fragmentation, client ACKs, CUBIC congestion control, retransmission, file-streaming FapiClient calls) shipped; next is 8.4.2 (FileCipher); 8.3 (application/token/proof/protocol lifecycle) still pending.
...
| 8.4.2 | **FCCore: `FileCipher` (Java parity).** DISK cipher-file format = UTF-8 JSON header (CryptoDataStr fields: `type:Symkey`, `alg:FC_AesGcm256_No1_NrC7`, `iv` hex — no `cipher` field) immediately followed by raw AES-256-GCM ciphertext‖tag, single GCM pass over the whole file. Header parsing = `readOneJsonFromInputStream` brace-matching parity. Plus streaming `Hash.sha256x2(fileURL:)` (1 MiB chunks). GCM is one-shot (CryptoKit) — Android buffers the full ciphertext on decrypt too, so the memory profile matches; fine on desktop, note the ceiling. Golden tests: jshell-generated Java vectors, encrypt-Java→decrypt-Swift and reverse (the `SignedMessage` playbook). | 0.5d |
