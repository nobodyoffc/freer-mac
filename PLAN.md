...
| 8.4.6 | **UI: Files pane** (`DataActivity` + `HatFileOpener` port) — ✅ shipped 2026-08-13, see the header blockquote. List with per-row location status, live search, drag-drop + panel registration, open/reveal/upload/download/remove actions with progress, detail + export + import sheets. Not yet eyeballed in a running window (see the note in the blockquote). | ✅ |
