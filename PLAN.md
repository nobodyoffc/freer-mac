...
status: Phases 1–7 complete; Phase 8 at 8.4.3 — FUDP large-payload streaming (8.4.1), the DISK `FileCipher` (8.4.2) and the `Hat` model + `HatsStore` (8.4.3) shipped, all three byte-checked against the real FC-AJDK classes; next is 8.4.4 (`FileVault`, reference-mode file layer); 8.3 (application/token/proof/protocol lifecycle) still pending.
