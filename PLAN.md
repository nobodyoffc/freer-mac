...
status: Phases 1–7 complete; Phase 8 at 8.1 — Tools pane (Encrypt / Decrypt / Sign / Verify / Hash / Random) shipped over new FCCore primitives (Base32, TOTP, Keccak-256, MD5/SHA1/HMAC, Bitcoin signed messages) and FCDomain `MsgSignature` + `TextCipher`; next is 8.2 (Secrets + TOTP cards)
...
| 8.2 | Secrets — `SecretsStore` + on-chain FEIP Secret sync/carve (mirror the 7.6.2b/7.6.3 contact machinery), `SecretView` (create / view / delete / import), TOTP cards (30 s rotation, eye-toggle, copy). | |
